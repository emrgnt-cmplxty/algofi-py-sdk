...
def prepare_remove_collateral_transactions(sender_addr, sender_key, params, amt, asset_name):
    (txn0, txn1, txn2, txn3) = get_init_txns(sender_addr, params)
    txn4 = ApplicationNoOpTxn(sender_addr, params, manager_id, [b'remove_collateral'])
    txn5 = ApplicationNoOpTxn(sender_addr, params, storage_ids[asset_name], [], foreign_apps=[manager_id])
    txn6 = AssetTransferTxn(sender=escrow_hashes[asset_name], sp=params, receiver=sender_addr, amt=amt, index=assets["b"+asset_name])
